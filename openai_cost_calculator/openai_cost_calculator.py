...
def calculate_cost(model_name=None, input_tokens=0, output_tokens=0):
    ...
    app_logger.info(f"Calculating cost for model: {model_name}")
    ...
    app_logger.info(f"Model name after processing: {model_name}")
    # Try to match the model name
    pricing_info = model_pricing.get(model_name)

    if pricing_info is None:
        app_logger.error(f"No pricing information found for model: {model_name}")
